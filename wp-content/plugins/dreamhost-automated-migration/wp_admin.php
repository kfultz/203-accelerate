...
class DHWPAdmin {
	...
	public function dhsecAdminMenu($hook) {
		if ($hook === 'toplevel_page_dreamhost') {
			wp_register_style('bvdh_normalize', plugins_url('assets/css/normalize.css',__FILE__ ));
			wp_register_style('bvdh_skeleton', plugins_url('assets/css/skeleton.css',__FILE__ ));
			wp_register_style('bvdh_form-styles', plugins_url('assets/css/form-styles.css',__FILE__ ));
...
<?php
	...
	public function adminPage() {
		...
		$form_to_load = 'token_form.php';
		if (isset($_REQUEST["ftp_details"])) {
			$form_to_load = 'ftp_form.php';
		}
		require_once dirname( __FILE__ ) . '/admin/' . $form_to_load;
	}
